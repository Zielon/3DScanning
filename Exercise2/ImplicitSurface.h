...
public:
    ...
    double Eval(const Eigen::Vector3d &_x) {
        ...
        /*(_x.x()-m_center.x())*(_x.x()-m_center.x())+(_x.y()-m_center.y())*(_x.y()-m_center.y())+
                (_x.z()-m_center.z())*(_x.z()-m_center.z())-m_radius*m_radius;*/
...
public:
    ...
    double Eval(const Eigen::Vector3d &_x) {
        ...
        double a = (_x - m_center).transpose() * (_x - m_center) + m_radius * m_radius - m_a * m_a;
        double xy2 =
                (_x.x() - m_center.x()) * (_x.x() - m_center.x()) + (_x.y() - m_center.y()) * (_x.y() - m_center.y());

        return a * a - 4 * m_radius * m_radius * xy2;
    }
...
public:
    ...
    double Eval(const Eigen::Vector3d &_x) {
        ...
        auto d = m_coefficents[m_numCenters + 3];

        for (int i = 0; i < m_numCenters; i++) {
            auto a = m_coefficents[i];
            double kernel = EvalBasis((m_funcSamp.m_pos[i] - _x).norm());
            result += a * kernel;
        }

        return result + b.dot(_x) + d;
    }
...
#define phi(i, j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
    ...
    void BuildSystem() {
        ...
        for (int i = 0; i < 2 * m_numCenters; i++) {
            ...
            if(i >= m_numCenters) b(i) = -0.01f;

            for (int o = 0; o < 3; o++) A(i, m_numCenters + o) = m_funcSamp.m_pos[i][o];
