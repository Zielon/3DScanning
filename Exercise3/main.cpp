...
void debugCorrespondenceMatching() {
    ...
    for (unsigned i = 0; i < 100; ++i) { // sourcePoints.size()
        ...
        if (match.idx >= 0) {
            ...
            resultingMesh = SimpleMesh::joinMeshes(SimpleMesh::cylinder(sourcePoint, targetPoint, 0.002f, 2, 15),
...
int alignBunnyWithProcrustes() {
    ...
    for (const auto &targetPoint : targetPoints) {
        resultingMesh = SimpleMesh::joinMeshes(SimpleMesh::sphere(targetPoint, 0.002f), resultingMesh,
                                               Matrix4f::Identity());
    }
