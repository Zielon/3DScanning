...
public:
	...
	Matrix4f estimatePose(const std::vector<Vector3f>& source, std::vector<Vector3f>& target, Matrix4f initialPose = Matrix4f::Identity()) {
		...
		//	pruneCorrespondences(transformedNormals, target.getNormals(), matches); // we dont have normals
		...
		//	std::vector<Vector3f> targetNormals;
			...
			for (int i = 0; i < source.size(); ++i)
			{
				if (matches[i].idx != -1)
				{
					...
					targetPoints.push_back(target[matches[i].idx]);
			...
			// Estimate the new pose
 			if (m_bUsePointToPlaneConstraints) {
				//FIXME: we dont have normals right now
...
private:
	...
	Matrix4f estimatePosePointToPoint(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints) {
		...
		// Build the system
		MatrixXf A = MatrixXf::Zero(3 * nPoints, 6);
		VectorXf b = VectorXf::Zero(3 * nPoints);
