...
public:
    ...
    bool loadMesh(const std::string &filename) {
        ...
        if (std::string(string1).compare("COFF") == 0) {
            ...
            for (unsigned int i = 0; i < numV; i++) {
                ...
                v.color = Vector4uc((unsigned char) colorInt.x(), (unsigned char) colorInt.y(),
        ...
        for (unsigned int i = 0; i < m_vertices.size(); i++) {
            ...
            if (vertex.position.allFinite())
                outFile << vertex.position.x() << " " << vertex.position.y() << " " << vertex.position.z() << " "
                        << int(vertex.color.x()) << " " << int(vertex.color.y()) << " " << int(vertex.color.z()) << " "
     ...
     */
    static SimpleMesh
    joinMeshes(const SimpleMesh &mesh1, const SimpleMesh &mesh2, Matrix4f pose1to2 = Matrix4f::Identity()) {
    ...
    static SimpleMesh sphere(Vector3f center, float scale = 1.f, Vector4uc color = {0, 0, 255, 255}) {
        ...
        const std::vector<double> vertexComponents = {-0.525731, 0, 0.850651, 0.525731, 0, 0.850651, -0.525731, 0,
                                                      -0.850651, 0.525731, 0, -0.850651, 0, 0.850651, 0.525731, 0,
                                                      0.850651, -0.525731, 0,
                                                      -0.850651, 0.525731, 0, -0.850651, -0.525731, 0.850651, 0.525731,
                                                      0, -0.850651, 0.525731, 0, 0.850651, -0.525731, 0, -0.850651,
                                                      -0.525731, 0};
        const std::vector<unsigned> faceIndices = {0, 4, 1, 0, 9, 4, 9, 5, 4, 4, 5, 8, 4, 8, 1, 8, 10, 1, 8, 3, 10, 5,
                                                   3, 8, 5, 2, 3, 2, 7, 3, 7, 10,
                                                   3, 7, 6, 10, 7, 11, 6, 11, 0, 6, 0, 1, 6, 6, 1, 10, 9, 0, 11, 9, 11,
                                                   2, 9, 2, 5, 7, 2, 11};

        // Add vertices.
        for (int i = 0; i < 12; ++i) {
            Vertex v;
            v.position = centerHomogenous + scale * Vector4f{float(vertexComponents[3 * i + 0]),
    ...
    static SimpleMesh camera(const Matrix4f &cameraPose, float scale = 1.f, Vector4uc color = {255, 0, 0, 255}) {
        ...
        // These are precomputed values for sphere aproximation.
        std::vector<double> vertexComponents = {25, 25, 0, -50, 50, 100, 49.99986, 49.9922, 99.99993, -24.99998,
                                                25.00426, 0.005185,
                                                25.00261, -25.00023, 0.004757, 49.99226, -49.99986, 99.99997, -50, -50,
                                                100, -25.00449, -25.00492, 0.019877};
        const std::vector<unsigned> faceIndices = {1, 2, 3, 2, 0, 3, 2, 5, 4, 4, 0, 2, 5, 6, 7, 7, 4, 5, 6, 1, 7, 1, 3,
                                                   7, 3, 0, 4, 7, 3, 4, 5, 2, 1, 5, 1, 6};

        // Add vertices.
        for (int i = 0; i < 8; ++i) {
            Vertex v;
            v.position = cameraToWorld * Vector4f{scale * float(vertexComponents[3 * i + 0]),
     ...
     */
    static SimpleMesh cylinder(const Vector3f &p0, const Vector3f &p1, float radius, unsigned stacks, unsigned slices,
                               const Vector4uc color = Vector4uc{0, 0, 255, 255}) {
        SimpleMesh mesh;
        ...
        for (unsigned i = 0; i <= stacks; i++)
            for (unsigned i2 = 0; i2 < slices; i2++) {
                ...
                v.position = Vector4f{p0.x() + radius * cosf(theta), p0.y() + radius * sinf(theta),
...
private:
    ...
    static Matrix3f face(const Vector3f &vA, const Vector3f &vB) {
        ...
        Matrix3f rotation = Matrix3f::Identity() + sinf(angle) * skewSymetricMatrix +
