...
public:
    ...
    PointCloud(const SimpleMesh &mesh) {
        ...
        for (size_t i = 0; i < nTriangles; i++) {
            ...
            Vector3f faceNormal = (m_points[triangle.idx1] - m_points[triangle.idx0]).cross(
                    m_points[triangle.idx2] - m_points[triangle.idx0]);
    ...
    PointCloud(float *depthMap, const Matrix3f &depthIntrinsics, const Matrix4f &depthExtrinsics, const unsigned width,
               const unsigned height, unsigned downsampleFactor = 1, float maxDistance = 0.1f) {
        ...
        for (int v = 0; v < height; ++v) {
            ...
            for (int u = 0; u < width; ++u) {
                ...
                if (depth == MINF) {
                    ...
                    pointsTmp[idx] = rotationInv * Vector3f((u - cX) / fovX * depth, (v - cY) / fovY * depth, depth) +
                                     translationInv;
                }
            }
        }
...
#pragma omp parallel for
        for (int v = 1; v < height - 1; ++v) {
            for (int u = 1; u < width - 1; ++u) {
                ...
                if (!std::isfinite(du) || !std::isfinite(dv) || abs(du) > maxDistanceHalved ||
                ...
                unsigned int idxBottom = (v - 1) * width + u;
                unsigned int idxTop = (v + 1) * width + u;
                unsigned int idxLeft = v * width + u - 1;
                unsigned int idxRight = v * width + u + 1;

                const Vector3f diffX = pointsTmp[idxRight] - pointsTmp[idxLeft];
                const Vector3f diffY = pointsTmp[idxTop] - pointsTmp[idxBottom];
                normalsTmp[idx] = -diffX.cross(diffY);

                //float nx = fovX * du;
                //float ny = fovY * dv;
                //float nz = -depthMap[idx] - (u - cX) * du + (v - cY) * dv;
                //normalsTmp[idx] = Vector3f(nx, ny, nz);
